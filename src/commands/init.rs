...
pub fn execute(args: &InitArgs) -> Result<()> {
    ...
    // 7. Generate MCP wrapper script and update .mcp.json
    if effective_here {
        // Wrapper script: uses $(dirname "$0") so it always cd's to the project root
        // before exec-ing the real binary — no absolute paths needed.
        let wrapper_path = cwd.join("stackydo-mcp.sh");
        if !wrapper_path.exists() {
            let script = concat!(
                "#!/bin/sh\n",
                "# Generated by `stackydo init --here`\n",
                "# Ensures stackydo-mcp starts from this project directory so that\n",
                "# config discovery works regardless of where the MCP client launches it.\n",
                "cd -- \"$(dirname -- \"$0\")\" && exec stackydo-mcp \"$@\"\n",
            );
            ...
            created.push("Created stackydo-mcp.sh (MCP wrapper script)".to_string());
        } else {
            created.push("stackydo-mcp.sh already exists".to_string());
        }

        // Create or update .mcp.json — only touch the "stackydo" server entry.
        let mcp_path = cwd.join(".mcp.json");
        let server_entry = serde_json::json!({
            "type": "stdio",
            "command": "./stackydo-mcp.sh"
        });
        ...
        fs::write(&mcp_path, format!("{pretty}\n"))?;

        created.push(if !existed {
            "Created .mcp.json".to_string()
        } else if already_had {
            "Updated 'stackydo' entry in .mcp.json".to_string()
        } else {
            "Added 'stackydo' entry to .mcp.json".to_string()
        });
    }
    ...
    // 9. Hint about how to use the workspace
